...
{
	float value = Joystick::AxisState(joystick, axis);
...
{
	std::string keyName = GetConfig()->String(bindName.c_str());
	if (keyName.length() == 0) {
		...
		GetConfig()->SetString(bindName.c_str(), keyName.c_str());
	}
...
{
	std::string axisName = GetConfig()->String(bindName.c_str());
	if (axisName.length() == 0) {
		axisName = defaultAxis;
		GetConfig()->SetString(bindName.c_str(), axisName.c_str());
	}
