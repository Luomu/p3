...
void GeoPatchContext::Cleanup() {
	...
	for (int i=0; i<NUM_INDEX_LISTS; i++) {
		if (indices_list[i]) {
			glDeleteBuffers(1, &indices_list[i]);
		}
	}
	delete [] vbotemp;
}
...
void GeoPatchContext::Init() {
	...
	for( int i=0; i<NUM_INDEX_LISTS; ++i ) {
		glGenBuffers(1, &indices_list[i]);
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, indices_list[i]);
		glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(unsigned short)*indices_tri_counts[i]*3, &(pl_short[i][0]), GL_STATIC_DRAW);
