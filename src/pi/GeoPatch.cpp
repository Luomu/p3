...
void GeoPatch::_UpdateVBOs() {
	if (m_needUpdateVBOs) {
		...
		glBufferData(GL_ARRAY_BUFFER, sizeof(GeoPatchContext::VBOVertex)*ctx->NUMVERTICES(), 0, GL_DYNAMIC_DRAW);
		...
		glBufferData(GL_ARRAY_BUFFER, sizeof(GeoPatchContext::VBOVertex)*ctx->NUMVERTICES(), ctx->vbotemp, GL_DYNAMIC_DRAW);
...
void GeoPatch::Render(Graphics::Renderer *renderer, const vector3d &campos, const matrix4x4d &modelView, const Graphics::Frustum &frustum) {
	if (kids[0]) {
		...
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, ctx->indices_vbo);
